...
@Getter
@Setter
public class PacketPlayOutSpawnPlayer extends PacketPlayOutEntity implements PacketPlayOut {
	private UUID uuid;
	...
	public PacketPlayOutSpawnPlayer(int id, GameProfile profile, Location loc, int hand, DataWatcher w) {
		this.setEntityId(id);
	...
	public PacketPlayOutSpawnPlayer(int id, UUID uuid, Location loc, int hand, DataWatcher w) {
		this.setEntityId(id);
		this.uuid = uuid;
		this.loc = loc;
		this.yaw = (byte) (int) (loc.getYaw() * 256.0F / 360.0F);
