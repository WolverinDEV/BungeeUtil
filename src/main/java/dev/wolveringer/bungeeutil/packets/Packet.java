...
public abstract class Packet {
	...
	public ByteBuf getByteBuf(ClientVersion version) {
		this.version = version;
		this.compressedId = getPacketId(version.getProtocollVersion(), this.getClass());
		return this.writeToByteBuff(null, version);
	}

	public Direction getDirection(){
		if (this.compressedId == -1) {
			this.compressedId = getPacketId(this.version.getProtocollVersion(), this.getClass());
		}
		return getDirection(this.compressedId);
	}
	...
	public int getPacketId(){
		...
		return getPacketId(this.compressedId);
	}
	...
	public Protocol getProtocol() {
		...
		return getProtocoll(this.compressedId);
	}
	...
	public ByteBuf writeToByteBuff(ByteBuf buf, ClientVersion version) {
		if(getPacketId(this.compressedId) < 0) {
			throw new RuntimeException("Unexpected packet id ("+getPacketId(this.compressedId) +")");
		}
		...
		if (buf instanceof PacketDataSerializer) {
			...
			s = PacketDataSerializer.create(getPacketId(this.compressedId), version, buf);
		}
