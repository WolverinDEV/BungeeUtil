...
public class Scroller {
	...
	public Scroller(String message, int width, int spaceBetween, char colourChar) {
		...
		for(int i = 0;i < raw.length();i++){
			...
			this.list.add(color + StringUntils.subStringWithoutChatcolors(msg, i > msg.length() ? msg.length() : i, i + width > msg.length() ? msg.length() : i + width) + StringUntils.subStringWithoutChatcolors(msg, 0, i + width > msg.length() ? i + width - msg.length() : 0));
		}
	}

	public String next() {
		return this.list.get(this.position++ % this.list.size()).substring(0);
	}

	@Override
	public String toString() {
		return "Scroller [position=" + this.position + ", list=" + this.list + ", colour=" + this.colour + "]";
	}
}